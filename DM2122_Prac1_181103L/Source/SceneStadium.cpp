...
void SceneText::Init()
{
	...
	camera.Init(Vector3(0, 2, 0), Vector3(0, 2, 20), Vector3(0, 1, 0));
	...
	meshList[GEO_GROUND]->textureID = LoadTGA("Image//football.tga");
...
void SceneText::RenderStadium(MS &modelStack, MS &projectionStack, MS &viewStack, Mtx44 &MVP)
{
	...
	modelStack.PushMatrix();
		...
		RenderMesh(meshList[GEO_CHARACTER], LightOn);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
		modelStack.Translate(0.f, 0.012f, 0.f);
		modelStack.Scale(27.028f, 1, 69.793f);
