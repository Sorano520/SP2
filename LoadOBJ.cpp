...
)
{
	...
	while (!fileStream.eof()) {
		...
		if (strncmp("v ", buf, 2) == 0) //process vertex position(Check next Slide)
		{
			Position vertex;
		...
		else if (strncmp("vn ", buf, 3) == 0) //process normal(Check next Slide)
		{
			Vector3 normal;
			sscanf_s((buf + 2), "%f%f%f", &normal.x, &normal.y, &normal.z);
			temp_normals.push_back(normal);
		}
		else if (strncmp("f ", buf, 2) == 0) //process face(Check next Slide)
		{ 
